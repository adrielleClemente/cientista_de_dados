...
def main():
    ...
    if data_file_1 is not None:
        ...
        # Verifique se a coluna 'y' existe antes de prosseguir
        if 'y' in bank_raw.columns and 'y' in bank.columns:
            bank_raw_target_perc = bank_raw.y.value_counts(normalize=True).to_frame() * 100
            bank_raw_target_perc = bank_raw_target_perc.reset_index()
            bank_raw_target_perc.columns = ['y', 'Proporção']

            try:
                bank_target_perc = bank.y.value_counts(normalize=True).to_frame() * 100
                bank_target_perc = bank_target_perc.reset_index()
                bank_target_perc.columns = ['y', 'Proporção']
            except KeyError:
                st.error('Erro no filtro: a coluna "y" não foi encontrada após os filtros.')
            ...
            if graph_type == 'Barras':
                sns.barplot(x=bank_raw_target_perc['y'], y=bank_raw_target_perc['Proporção'], ax=ax[0])
                ax[0].bar_label(ax[0].containers[0])
                ax[0].set_title('Dados brutos', fontweight="bold")
                
                sns.barplot(x=bank_target_perc['y'], y=bank_target_perc['Proporção'], ax=ax[1])
                ax[1].bar_label(ax[1].containers[0])
                ax[1].set_title('Dados filtrados', fontweight="bold")
            else:
                bank_raw_target_perc.plot(kind='pie', y='Proporção', labels=bank_raw_target_perc['y'], autopct='%.2f', ax=ax[0])
                ax[0].set_title('Dados brutos', fontweight="bold")
                
                bank_target_perc.plot(kind='pie', y='Proporção', labels=bank_target_perc['y'], autopct='%.2f', ax=ax[1])
                ax[1].set_title('Dados filtrados', fontweight="bold")
